...
int main (int argc, char *argv[])
{
  ...
  tS = now(); // start time for whole loop
  ...
  tE = now();	// time at end of loop

  // final report
  if (USE_GPU) {
    printf("CUDA kernel avg duration: %.8f seconds \n", cuda_t/( GRID_DIM * GRID_DIM * GRID_DIM * PHYSICAL_PARAMS * RK));
    printf("CUDA kernel total duration: %.8f seconds \n", cuda_t);
  }
  else {
    printf( "SERIAL kernel  avg duration: %.8f seconds \n", serial_t/( GRID_DIM * GRID_DIM * GRID_DIM * PHYSICAL_PARAMS * RK)); 
    printf("SERIAL FINAL total duration: %.8f seconds \n", serial_t); 
  }
  printf("Total kernel iterations: %d \n", ( GRID_DIM * GRID_DIM * GRID_DIM * PHYSICAL_PARAMS * RK));
  printf("Total time for grid dim %d and element dim %d : %f \n",GRID_DIM,ELEMENT_SIZE, tdiff(tS, tE));
